...
@Component({
  selector: 'app-pomodoro-quiz',
  templateUrl: './pomodoro-quiz.component.html',
  styleUrls: ['./pomodoro-quiz.component.css']
})
export class PomodoroQuizComponent {
  public firstName = '';
  public lastName = '';
  //Chọn môn học
  ...
  SubjectSelect() {
    if (this.QuizSelect.valid) {
      ...
      this.quiz.PomodoroQuiz(selectedID).subscribe({
        next: (res) => {
          this.quiz.qns = res;
        },
      });
